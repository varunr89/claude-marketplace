...
_DEFAULT_HISTORY_PATH = None  # Computed lazily
...
def load_voice_history(history_path: Optional[str] = None) -> list[str]:
    ...
    path = Path(history_path) if history_path else _default_data_dir() / "voice_history.json"
    if not path.exists():
        return []
    try:
...
def pick_voices(
    ...
    Args:
        ...
        history_path: path to voice_history.json (default: $PODCAST_DATA_DIR/...)
